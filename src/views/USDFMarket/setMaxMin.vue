	...
	export default {
		...
		methods: {
			getCurrentPrice(){
				...
				let url = this.baseUrl +'/cms_base/udeal_scope'
				this.$http.get(url,params)
				.then((res)=>{
					    
						if (res.body.status ==0){
							console.log(res.body)
							_this.form.cny = res.body.data.min
							_this.form.usdt = res.body.data.max
			...
			onSubmit() {
				...
				this.$confirm('确认提交吗？', '提示', {}).then(() => {
					let params ={
						params: {
							min: _this.form.cny ,   // 类型 
							max: _this.form.usdt
						}
					}
					this.listLoading = true;
					let url = this.baseUrl +'/cms_base/update_udeal_scope'
					this.$http.get(url,params)
					.then((res)=>{
